 ...
 */
class wxcert_controller extends common
{
	function set_search(){
	...
	function index_action()
	{
		...
		$count=$this->obj->DB_select_alls("wx_cert","company","a.`uid`=b.`uid` and a.`type`='3' $where","a.uid");
 		$num = count($count);
 		$page = new page($page,$limit,$num,$pageurl);
		$pagenav=$page->numPage();
		$rows=$this->obj->DB_select_alls("wx_cert","company","a.`uid`=b.`uid` and a.`type`='3' $where $order limit $ststrsql,$limit","b.name,a.*");
		$this->yunset("pagenav",$pagenav);
		$this->yunset("rows",$rows);
		$this->yuntpl(array('admin/admin_wx_cert'));
	}
	function sbody_action(){
		$userinfo = $this->obj->DB_select_once("wx_cert","`type`='3' and `uid`='".$_POST['uid']."'","`statusbody`");
	...
	function status_action()
	{
		if($_POST['uid'])
		{
			...
			$id=$this->obj->DB_update_all("wx_cert","`status`='".$_POST['status']."',`statusbody`='".$_POST['statusbody']."'","`uid` in (".$uid.") and `type`='3'");
	...
	function del_action()
	{
	    ...
	    $cert=$this->obj->DB_select_all("wx_cert","`uid` in (".$linkid.") and `type`='3'","`check`");
	    if(is_array($cert))
	    {
	     	foreach($cert as $v)
	     	{
		...
		$delid=$this->obj->DB_delete_all("wx_cert","`uid` in (".$linkid.")  and `type`='3'","");
