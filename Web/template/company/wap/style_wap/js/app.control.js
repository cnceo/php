...
function loadPageURL_ios(url, localUrl, bool) {
    ...
    if (bool == 0) {
        ...
        if (window.location.href.indexOf("&loadUrl") == -1) {
            ...
            uri = getResponseText(localUrl + 'ios&ios=cache');
    ...
    fileTransfer.download(uri, tar, function (entry) {
        window.location.href = tar + "?from=ios" + '&reNum=' + reNum + loadUrl + "&ios=cache";
    }, function (error) {
        alert("加载失败！");
    });
}
...
$(function () {
    var start_pageX = 0, start_pageY = 0;
    ...
    $body.delegate('a:not(".notcut,.opendialog,openfloat,.openslide,.cutJump,.cutjump,.video"), .cutauchor', clickEvent, function (e) {
        if (clickEvent == "touchend" && (start_pageX > 0 || start_pageY > 0)) {
            start_pageX = 0;
            start_pageY = 0;
            return false;
        }
...
function cutAuchor(href, jTarget, flag) {
    ...
    try {
        if (/from2=(editview|weidbview)/gi.test(location.href) == true) {
            ...
            //整理href
            href = href.replace(/(?:^http:\/\/m\.[^.]+\.quickapp\.cn)|(?:dname=yes)|(?:from2=editview)|(?:from2=weidbview)|(?:client=m)|(?:return=1)/gi, "").replace(/\&+/gi, "&").replace(/\?\&+/gi, "?");
...
function setClassDefault(obj) {
    obj.removeClass("abox-active");
}
...
function ImgLarge(obj) {
    if (obj.attr("class").indexOf("no_img") != -1) return;
