...
$(function () {
	...
	*/
	...
	try {
		...
		function handleInput(arr1, arr2) {
		...
		$("iframe").load(function() {
			$(window.frames["ChinaECFrame1"].document).find("body").on("touchmove",
			function(ev) {
			...
			$(window.frames["ChinaECFrame1"].window.frames["Form1"].document).find("body").on("touchmove",
			function(ev) {
				ev.preventDefault();
	
			})
	
			var input_list = $(window.frames["ChinaECFrame1"].window.frames["Form1"]).find("input");
			var textarea_list = $(window.frames["ChinaECFrame1"].window.frames["Form1"]).find("textarea");
			handleInput(input_list, textarea_list);
		})
	} catch(e) {};
});
...
function weixin() {
    ...
    //    goToBrowser("/common/qm/http/redirectApp.html?url=" + encodeURIComponent('http://s.jiathis.com/?webid=weixin&uid=1&jtss=0&appkey=&ralateuid=&url=' + url + '&title=' + txt + '&isexit=false'));
...
function sms_to() {
    ...
    if (navigator.userAgent.match(/micromessenger/gi) == null) {
        ...
        //    window.top.location.href=("/"+_sys_idcorp+"_"+_sys_idinfo+"_8.html#mp.weixin.qq.com");
    }
};
...
$(function () {
    ...
    $(".home-function-2").click(function () {
        if (clicking == true)
            return false;
...
$(function () {
    //管理员登录检查
    var url="/CorpSvc/Temp/StandardPage/Admini.aspx?idCorp=[idcorp]&type=dl&t=" + new Date().getTime();
...
function FreshNewOrder() {
    $.getJSON("/Trade/MobileProcess/json/GetUnread.ashx?t=" + new Date().getTime(), function (data) {
...
function FreshShopCart() {
    $.getJSON("/common/getCustomer.ashx?t=" + new Date().getTime(), function (data) {
...
$(function () {
    ...
    function liuyan_click() {
        $("#new-msg-liuyan").parent().one("click", function () {
    ...
    function glydg_click() {
        $("#new-msg-glydg").parent().one("click", function () {
    ...
    function glyzx_click() {
        $("#new-msg-glyzx").parent().one("click", function () {
    ...
    function hydg_click() {
        $("#new-msg-hydg").parent().one("click", function () {
    ...
    function hyzx_click() {
        $("#new-msg-hyzx").parent().one("click", function () {
    ...
    function getNewMsgCount(className, qf, qv, function_callback) {
        ...
        if ($target.length > 0) {
            $.getJSON("/common/lyb/api/NewMessage.aspx", { "idcorp": _sys_idcorp, "p": iproject, "rf": read_field, "qf": qf, "qv": encodeURI(escape(qv)), "v": Math.random() }, function (data) {
...
$(function () {
    //链接
    var title = "";
    var iZyjjUrl = "/corpsvc/temp/standardpage/infojs.aspx?idcorp=" + _sys_idcorp + "&type=tsxx&dname=yes&time=" + Math.random();
    var akng = 0;
    var title = "";
...
//    var key_sound = 'http://img.quickapp.cn/41/1100001491/sys/key_sound.wav';

//    var href = _this.attr("href");
//    if (typeof href != 'undefined' && href != '')
//        _this.removeClass("abox-active").addClass("abox-active");
...
//        }
//    }
//    if (/[?&]from=ios\b/gi.test(location.href)) {
//        if (onoff) {
//            try {
//                playAudio(key_sound);
//            } catch (e) {
...
function () {
    if (document.getElementById("album_url") == null) return;
