...
//    $.getJSON('http://' + secdomain + '/common/getCustomer.ashx?t=' + new Date().getTime(), function (data) {
...
function AddToShoppingCart(idCorp, idWare, idCustomer, source, $ele, callback, price) {
    ...
    if (re.test(url)) {
        ...
        mobilebuy(idCorp, idWare, idCustomer, source, $ele, callback);
    }
    else {
        if (!idCustomer) {
            //document.location.href = "/1800000624_10000004_8.html?url=" + escape(url);
            //if (idCorp != 1100005053) {
            document.location.href = "/" + idCorp + "_10000004_8.html?url=" + escape(url);
            return;
            //}
        }
        ...
        var url = "/Trade/AddToShoppingCart.aspx?parm=" + parm + "&Language=Cn&dname=yes";
        ...
        //buyWindow.Show();
        window.open("/Trade/AddToShoppingCart.aspx?parm=" + parm + "&Language=Cn&dname=yes", "AddToShoppingCart");
 ...
 * 手机网站点击购物以后执行
*/
function mobilebuy(idCorp, idWare, idCustomer, source, $ele, callback) {
    ...
    //        var url = "/localidcorp_10000061_5.html";
...
function AddToShoppingCart_En(idCorp, idWare) {
    var parm = GetParm(idCorp, idWare);
    window.open("/Trade/AddToShoppingCart.aspx?parm=" + parm + "&Language=En", "AddToShoppingCart");
}

function addtoshoppingcart(idCorp, idWare) {
    var parm = GetParm(idCorp, idWare);
    window.open("/Trade/AddToShoppingCart.aspx?parm=" + parm + "&Language=Cn", "AddToShoppingCart");
}
...
$(document).ready(function () {
    ...
    //console.log('load ShoppingCart.js');
    ...
    $('.buynow').bind(eventname, function (e) {
        ...
        AddToShoppingCart(idCorp, idWare, 0, 'buynow', $this, function () {
            //跳转到结算页
            //var url = "/localidcorp_0_8.html?sp=1100000112-20000050&dname=yes";

            //跳转到购物车
            var url = "/localidcorp_0_8.html?sp=1100000112-20000049&dname=yes";
...
$(function () {
    ...
    $('.item').each(function (idx, item) {
        $(item).click(function () {
            ...
            $('#itemprice').html('<img src="/app/gen/options/img/loading.gif" style="display: inline;height:15px;width:15px;">');

            $.getJSON('/CorpSvc/getWarePrice.ashx?idcorp=' + getIdCorp() + '&idware=' + getIdWare() + '&t=' + new Date().getTime(), function (sJson) {
                $('#itemprice').html('');
...
var calculate = function () {
    ...
    $('#btnBuyService').attr('href', '/Trade/AddToShoppingCart.aspx?idCorp=' + getIdCorp() + '&idWare=' + idWare + '&num=' + num + '&Language=Cn&dname=yes');
