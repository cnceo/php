...
function eventBackButton() {
    ...
    if (isIndex && /from2=weidbview/gi.test(location.href) == false) {
        exitApp();
    } else if (window.location.href.indexOf('push.html') != -1) {
        // back_index();
...
OutSide.prototype.outSide = function (url, win, fail) {
    cordova.exec(win, fail, "OutSidePlugin", "outSide", [url]);
};
...
CheckFile.prototype.checkFile = function (url, win, fail) {
    cordova.exec(win, fail, "CheckSdFile", "checkFile", [url]);
};
...
Downloader.prototype.downloadFile = function (fileUrl, params, localUrl, bool, win, fail) {
    if (!fail)
        win = params;
    try {
        cordova.exec(win, fail, "Downloader", "downloadFile", [fileUrl, params, localUrl, bool]);
    }
    catch (e) { }
};
...
UpdateVersion.prototype.CheckUpdate = function (win, fail) {
    cordova.exec(win, fail, "Update", "update", []);
}
...
var Pasteboard = {
    paste: function (success, fail, str) {
        cordova.exec(success, fail, "Pasteboard", "paste", [str]);
    }
...
DownApp.prototype.downApp = function (loadUrl, name, win, fail) {
    cordova.exec(win, fail, "DownApp", "downApp", [loadUrl, name]);
}
...
ToIndex.prototype.backToIndex = function (win, fail) {
    cordova.exec(win, fail, "ToIndexPlugin", "toIndex", []);
}
...
CloseApp.prototype.closeApp = function (win, fail) {
    cordova.exec(win, fail, "CloseAppPlugin", "closeApp", []);
}
...
function getResponseText(url) {
    ...
    var down = page_request.responseText;
    return down;
...
function getClickEvent() {
    ...
    var clickEvent = system.win || system.mac || system.xll
        || system.android_app
...
var clickEvent = getClickEvent(), clicking = false;
...
function loadSound() {
    ...
    if (/ios|iphone|ipad|ipod/gi.test(navigator.userAgent) == true) {
        ...
        player.src = "http://img.quickapp.cn/" + (_sys_idcorp % 50) + "/" + _sys_idcorp + "/sys/key_sound.wav";
    }
...
$(function () {
    ...
    $body.delegate('.abox', clickEvent, function (e) {
        //CLICK_HAVE_SLIDE 不能置为false,会影响到跳转时的滑动判断
        if (CLICK_HAVE_SLIDE == true)
            return;
    ...
    //大部分点击跳转事件在此处理
    $body.delegate('a:not(".notcut,.opendialog,openfloat,.openslide,.cutJump,.cutjump,.video"), .cutauchor', clickEvent, function (e) {
        if (CLICK_HAVE_SLIDE == true)
            return;
        ...
        //微店宝中打开子级页面
        if (typeof WeiDB != "undefined"
            && /\bblank\b/gi.test($(this).attr("class")) == true
            && (/from2=weidbview/gi.test(location.href) == false
                || ($(this).attr("class").indexOf("this-edit") == -1
                && $(this).parents(".this-edit").length == 0)
            )) {
            //整理href
            href = href.replace(/(?:^http:\/\/m\.[^.]+\.quickapp\.cn)|(?:dname=yes)|(?:from2=weidbview)|(?:client=m)|(?:return=1)/gi, "").replace(/\&+/gi, "&").replace(/\?\&+/gi, "?");

            //首页时
            if (href == "/" || href == "" || href.indexOf("?") == 0) {
                if (typeof top.adminidcorp != "undefined") {
                    href = "/corpsvc/temp/t/mdefault.aspx?idcorp=" + top.adminidcorp + "&dname=yes&from2=weidbview&client=m&return=1";
                }
            }
            else {//子页时
                href += href.indexOf("?") == -1 ? "?dname=yes&from2=weidbview&client=m&return=1" : "&dname=yes&from2=weidbview&client=m&return=1";
            }

            WeiDB.mini.load(href);
            return;
        }
    ...
    $body.delegate('.cutjump,.cutJump', clickEvent, function (e) {
        var href = encodeURI($(this).attr("href"));
        var url = "/common/qm/http/redirectApp.html?title=" + encodeURI(escape(document.title)) + "&url=" + href;
        goToBrowser(url);
        e.preventDefault();
    });
            ...
            if (from2_type == "editview") {
                ...
                if (href == "/" || href == "" || href.indexOf("?") == 0) {
                    if (typeof top._sys_admin_idcorp != "undefined") {
                        href = "/corpsvc/temp/t/mdefault.aspx?idcorp=" + top._sys_admin_idcorp + "&dname=yes&from2=" + from2_type + "&client=m&return=1";
                    }
                }
                else {//子页时
    ...
    if (typeof result != "undefined" && result.uri.indexOf("http://") == 0) {
        if (/[?&]from=(?:app|ios)\b/gi.test(result.uri) == false) {
            if (/^http:\/\/m\.[^./]+\.quickapp\.cn\/?$/gi.test(result.uri) == true)
                result.uri += "/corpsvc/temp/t/mdefault.aspx?idcorp=" + _sys_idcorp + "&client=m";
...
function ImgLarge(obj) {
    ...
    sRes += "#down-load{color:#fff;padding-top:30px;text-align:center;background-image:url(http://img.quickapp.cn/48/1100000048//down.png);background-position:-30px -10px;cursor:pointer;width:30px;height:30px;font-size:12px;position:absolute;z-index:221001;top:30px;left:30px;display:none}";
    sRes += "#close{background-image:url(/Import/Images/mask-close.png);background-position:-10px -10px;cursor:pointer;width:30px;height:30px;position:absolute;z-index:220001;top:30px;right:30px;}";
