...
*/
class expect_pool_controller extends company
{
	function index_action()
	{
		...
		if($_GET['keyword'])
		{
			...
			$where.=" and `body` like '%".$_GET['keyword']."%'";
			
			$urlarr['keyword']=$_GET['keyword'];
		}
		$this->public_action();
		$urlarr['c']='expect_pool';
		$urlarr["page"]="{{page}}";
		$pageurl=$this->url("index","index",$urlarr);
		$rows=$this->get_page("resume_file","$where  order by id desc",$pageurl,"10");
		...
		if(is_array($rows))
		{
			if(!$_GET['keyword'])
			{
				if(empty($resume))
				{
					...
					$resume=$this->obj->DB_select_alls("resume_file","","a.uid=b.uid and a.`r_status`<>'2' and a.uid in (".@implode(',',$uid).")","a.`title`");
				}
			}
